<script setup>
/**
 * Departments Index Page - Standardized Table UI
 */
...
watch(perPage, (val) => {
    ...
    router.get(route('departments.index'), { ...props.filters, per_page: val }, { 
        preserveScroll: true, 
        preserveState: true,
...
const openEdit = (item) => { editingItem.value = item; form.name = item.name; form.code = item.code; form.description = item.description || ''; showModal.value = true; };
const submit = () => editingItem.value ? form.put(route('departments.update', editingItem.value.id), { onSuccess: () => showModal.value = false }) : form.post(route('departments.store'), { onSuccess: () => showModal.value = false });
...
const toggleAll = (e) => {
    if (e.target.checked) {
        selectedItems.value = props.departments.data.map(i => i.id);
    } else {
        selectedItems.value = [];
    }
};
...
const bulkDelete = () => {
    confirmData.value = {
        title: 'Hapus Massal Departemen',
        message: `Hapus ${selectedItems.value.length} departemen yang dipilih?`,
        type: 'danger',
        onConfirm: () => {
            router.post(route('departments.bulk-delete'), { ids: selectedItems.value }, {
                onSuccess: () => {
                    selectedItems.value = [];
...
const toggleSort = () => {
    ...
    router.get(route('departments.index'), { ...props.filters, dir: dir }, { 
        preserveScroll: true, 
        preserveState: true,
        only: ['departments', 'filters']
    });
...
const deleteItem = (item) => {
    confirmData.value = {
        ...
        message: `Apakah Anda yakin ingin menghapus departemen "${item.name}"? Semua data yang terkait mungkin akan terpengaruh.`,
        type: 'danger',
        onConfirm: () => {
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 leading-tight">Manajemen Departemen</h2>
                    <p class="text-xs text-gray-500 mt-1">Kelola daftar divisi atau departemen di perusahaan.</p>
                </div>
            </div>
        </template>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-h-[600px] transition-opacity duration-300" :class="{'opacity-40 pointer-events-none': isLoading}">
                    ...
                    <div class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-gray-50 to-white">
                        <div class="flex items-center gap-3 shrink-0">
                            ...
                            <div class="hidden sm:block">
                                <h3 class="text-sm font-bold text-gray-800">Daftar Departemen</h3>
                                <p class="text-[10px] text-gray-400 font-medium">{{ departments.total || 0 }} departemen</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 flex-grow sm:justify-end">
                            <div class="relative flex-grow max-w-[300px]">
                                ...
                                <input v-model="search" type="text" placeholder="Cari departemen atau kode..." 
                    ...
                    <div v-if="departments.data?.length > 0" class="px-6 py-3 border-b border-gray-100">
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-3 rounded-xl transition-all border"
                            ...
                            <div class="flex items-center gap-3">
                                <label class="flex items-center gap-2 cursor-pointer select-none">
                                    <input type="checkbox" @change="toggleAll" :checked="selectedItems.length === departments.data?.length && departments.data?.length > 0"
                        ...
                        <table class="min-w-full text-left">
                            <thead class="bg-gray-50/80">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 border-b border-gray-100 w-24 text-center text-xs font-bold text-gray-500">Kode</th>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                <tr v-for="(item, index) in departments.data" :key="item.id" 
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-700 border border-gray-200 uppercase tracking-wider tabular-nums">
                                            {{ item.code }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                ...
                                <tr v-if="!departments.data?.length">
                                    <td colspan="6" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-xs font-semibold text-gray-400">Belum ada departemen</p>
                                            <p class="text-[10px] text-gray-300 mt-0.5">Silakan buat divisi baru</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div class="px-6 py-3.5 border-t border-gray-100 bg-gray-50/30 flex items-center justify-between mt-auto">
                        ...
                        <nav v-if="departments.links && departments.links.length > 3" class="flex gap-1">
                            <template v-for="(link, k) in departments.links" :key="k">
                ...
                <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-xl border border-gray-200 shadow-2xl w-full max-w-md overflow-hidden">
                        <div class="px-6 py-4 flex justify-between items-center bg-emerald-600 shadow-sm relative z-10">
                            <h3 class="text-sm font-bold text-white tracking-wide">{{ editingItem ? 'Edit Departemen' : 'Tambah Departemen' }}</h3>
                        ...
                        <form @submit.prevent="submit" class="p-6 space-y-4">
                            <div>
                                <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Kode *</label>
                                ...
                                <p v-if="form.errors.code" class="mt-2 text-[10px] font-bold text-rose-600 uppercase">{{ form.errors.code }}</p>
                            </div>
                            <div>
                                <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Nama Departemen *</label>
                            ...
                            <div>
                                ...
                                <textarea v-model="form.description" rows="3" 
                                    class="w-full rounded-lg border-gray-300 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition"
                                    placeholder="Deskripsi tugas divisi..."></textarea>
