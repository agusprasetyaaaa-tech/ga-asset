    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 leading-tight">Manajemen Pemeliharaan</h2>
                    <p class="text-xs text-gray-500 mt-1">Riwayat perbaikan, servis berkala, dan performa aset.</p>
                </div>
            </div>
        </template>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-h-[600px] transition-opacity duration-300" :class="{'opacity-40 pointer-events-none': isProcessing}">
                    ...
                    <div class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-gray-50 to-white">
                        <div class="flex items-center gap-3 shrink-0">
                            ...
                            <div class="hidden sm:block">
                                <h3 class="text-sm font-bold text-gray-800">Daftar Pemeliharaan</h3>
                                <p class="text-[10px] text-gray-400 font-medium whitespace-nowrap">{{ logs.total || 0 }} total data</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 flex-grow sm:justify-end">
                            ...
                            <div class="relative flex-grow max-w-[300px]">
                                ...
                                <input v-model="search" type="text" placeholder="Cari nama, vendor..." 
                    ...
                    <!-- Bulk Delete Area -->
                    <div v-if="logs.data.length > 0" class="px-6 py-3 border-b border-gray-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                        <div class="flex items-center gap-3 p-3 rounded-xl transition-all border w-full"
                            :class="selectedLogs.length > 0 ? 'bg-rose-50 border-rose-100' : 'bg-gray-50/50 border-gray-100'">
                             ...
                             <button v-if="selectedLogs.length > 0 && can('delete activity')" @click="bulkDelete" 
                        ...
                        <table class="min-w-full divide-y divide-gray-100 text-left table-auto">
                            <thead class="bg-gray-50/80">
                                <tr>
                                    ...
                                    <th @click="toggleSort('asset')" class="px-6 py-3 border-b border-gray-100 text-xs font-bold text-gray-500 cursor-pointer hover:bg-emerald-50 select-none group transition-all">
                                        <div class="flex items-center gap-1.5">
                                            Detail Aset
                                            <div class="flex flex-col -space-y-1 opacity-40 group-hover:opacity-100 transition-opacity">
                                                <svg :class="{'text-emerald-600 opacity-100': filters.sort === 'asset' && filters.dir === 'asc'}" class="h-2 w-2" fill="currentColor" viewBox="0 0 24 24"><path d="M12 5l-8 8h16l-8-8z"/></svg>
                                                <svg :class="{'text-emerald-600 opacity-100': filters.sort === 'asset' && filters.dir === 'desc'}" class="h-2 w-2" fill="currentColor" viewBox="0 0 24 24"><path d="M12 19l8-8H4l8 8z"/></svg>
                                            </div>
                                        </div>
                                    </th>
                                    <th class="px-6 py-3 border-b border-gray-100 text-xs font-bold text-gray-500">Keterangan & Vendor</th>
                                    <th class="px-6 py-3 border-b border-gray-100 text-center w-28 text-xs font-bold text-gray-500">Status</th>
                                    <th @click="toggleSort('cost')" class="px-6 py-3 border-b border-gray-100 text-xs font-bold text-gray-500 cursor-pointer hover:bg-emerald-50 select-none group transition-all w-40">
                                        <div class="flex items-center gap-1.5">
                                            Biaya
                                            <div class="flex flex-col -space-y-1 opacity-40 group-hover:opacity-100 transition-opacity">
                                                <svg :class="{'text-emerald-600 opacity-100': filters.sort === 'cost' && filters.dir === 'asc'}" class="h-2 w-2" fill="currentColor" viewBox="0 0 24 24"><path d="M12 5l-8 8h16l-8-8z"/></svg>
                                                <svg :class="{'text-emerald-600 opacity-100': filters.sort === 'cost' && filters.dir === 'desc'}" class="h-2 w-2" fill="currentColor" viewBox="0 0 24 24"><path d="M12 19l8-8H4l8 8z"/></svg>
                                            </div>
                                        </div>
                                    </th>
                                    <th class="px-6 py-3 border-b border-gray-100 text-center text-xs font-bold text-gray-500 hidden sm:table-cell w-32">Tanggal</th>
                                    <th class="px-6 py-3 border-b border-gray-100 text-right text-xs font-bold text-gray-500 w-32">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50 bg-white">
                                <tr v-for="(log, idx) in logs.data" :key="log.id" 
                                    class="hover:bg-emerald-50/30 transition-colors duration-150 group"
                                    :class="{'bg-emerald-50/50': selectedLogs.includes(log.id)}">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="min-w-0">
                                                <Link :href="route('assets.show', log.asset?.id || '#')" class="font-semibold text-sm text-gray-800 hover:text-emerald-600 truncate block">{{ log.asset?.name }}</Link>
                                                <div class="text-[10px] text-gray-400 leading-tight truncate max-w-sm mt-0.5">
                                                    <span :class="typeColor(log.type)" class="text-[9px] font-bold uppercase px-1 py-0.5 rounded border inline-block mr-1">{{ typeLabel(log.type) }}</span>
                                                    {{ log.asset?.asset_code || log.asset?.barcode_code }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="min-w-0">
                                            <div class="text-sm font-bold text-gray-800 truncate">{{ log.technician || '-' }}</div>
                                            <div class="text-[11px] text-gray-400 italic truncate max-w-xs mt-0.5">{{ log.description }}</div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <StatusBadge :status="log.status === 'selesai' ? 'available' : 'maintenance'" :custom-label="log.status" />
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="font-semibold text-sm text-gray-800">{{ formatCurrency(log.cost) }}</span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-right space-x-1 whitespace-nowrap">
                                        <button v-if="log.status === 'proses' && can('edit activity')" @click="completeMaintenance(log)" class="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg transition" title="Selesaikan"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg></button>
                                        <button v-if="can('edit activity')" @click="openEdit(log)" class="p-1.5 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                                        <button v-if="can('delete activity')" @click="deleteLog(log)" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                    </td>
                                </tr>
                    ...
                    <div class="md:hidden divide-y divide-gray-50">
                        <!-- Mobile Bulk Bar -->
                        <div v-if="logs.data.length > 0" class="p-4 flex items-center justify-between bg-gray-50/50 border-b border-gray-100">
                            <label class="flex items-center gap-2 cursor-pointer select-none">
                                <input type="checkbox" @change="toggleAll" :checked="selectedLogs.length === logs.data.length && logs.data.length > 0"
                                    class="h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500 transition" />
                                <span class="text-xs font-bold text-gray-600">Pilih Semua</span>
                                <span v-if="selectedLogs.length > 0" class="text-[10px] font-extrabold text-rose-600 ml-1">({{ selectedLogs.length }})</span>
                            </label>
                            <button v-if="selectedLogs.length > 0" @click="bulkDelete" 
                                class="flex items-center gap-1.5 px-3 py-1.5 bg-rose-600 text-white rounded-lg text-[10px] font-black active:scale-95 transition-all">
                                <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                Hapus
                            </button>
                        </div>
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[11px] text-emerald-700 font-mono font-bold">{{ log.asset?.asset_code || log.asset?.barcode_code }}</p>
                                    </div>
                                </div>
                            ...
                            <div class="grid grid-cols-2 gap-2 text-[11px]">
                                <div class="p-2 bg-gray-50 rounded-lg border border-gray-100">
                                    <span class="text-gray-400 block mb-0.5">Biaya</span>
                                        <div class="text-[13px] font-bold text-gray-900">{{ formatCurrency(log.cost) }}</div>
                                </div>
                                <div class="p-2 bg-gray-50 rounded-lg border border-gray-100">
                    ...
                    <div class="px-6 py-3.5 border-t border-gray-100 bg-gray-50/30 flex items-center justify-between mt-auto">
                        ...
                        <nav v-if="logs.links && logs.links.length > 3" class="flex gap-1">
