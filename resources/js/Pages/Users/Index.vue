<script setup>
/**
 * Users Index Page - Standardized Table UI
 */
...
watch(perPage, (val) => {
    ...
    router.get(route('users.index'), { ...props.filters, per_page: val }, { 
        preserveScroll: true, 
        preserveState: true,
        only: ['users', 'filters'],
...
const openCreate = () => { 
    ...
    form.reset(); 
    if (props.roles && props.roles.length > 0) form.role = props.roles[0].name;
...
const submit = () => {
    if (editingItem.value) {
        form.put(route('users.update', editingItem.value.id), { 
...
const toggleAll = (e) => {
    if (e.target.checked) {
        selectedItems.value = props.users.data.map(i => i.id);
    } else {
        selectedItems.value = [];
    }
};
...
const bulkDelete = () => {
    confirmData.value = {
        title: 'Hapus Massal Pengguna',
        message: `Hapus ${selectedItems.value.length} pengguna yang dipilih?`,
        type: 'danger',
        onConfirm: () => {
            router.post(route('users.bulk-delete'), { ids: selectedItems.value }, {
                onSuccess: () => {
                    selectedItems.value = [];
...
const toggleSort = () => {
    ...
    router.get(route('users.index'), { ...props.filters, dir: dir }, { 
        preserveScroll: true, 
        preserveState: true,
        only: ['users', 'filters']
    });
...
const deleteItem = (item) => {
    confirmData.value = {
        ...
        message: `Apakah Anda yakin ingin menghapus pengguna "${item.name}"?`,
        type: 'danger',
        onConfirm: () => {
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 leading-tight">Manajemen Pengguna</h2>
                    <p class="text-xs text-gray-500 mt-1">Kelola akun dan hak akses pengguna sistem.</p>
                </div>
            </div>
        </template>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-h-[600px] transition-opacity duration-300" :class="{'opacity-40 pointer-events-none': isLoading}">
                    ...
                    <div class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-gray-50 to-white">
                        <div class="flex items-center gap-3 shrink-0">
                            ...
                            <div class="hidden sm:block">
                                ...
                                <p class="text-[10px] text-gray-400 font-medium">{{ users.total || 0 }} pengguna</p>
                            </div>
                        </div>
                    ...
                    <div v-if="users.data?.length > 0" class="px-6 py-3 border-b border-gray-100">
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-3 rounded-xl transition-all border"
                            ...
                            <div class="flex items-center gap-3">
                                <label class="flex items-center gap-2 cursor-pointer select-none">
                                    <input type="checkbox" @change="toggleAll" :checked="selectedItems.length === users.data?.length && users.data?.length > 0"
                            ...
                            <button v-if="selectedItems.length > 0" @click="bulkDelete"
                        ...
                        <table class="min-w-full text-left">
                            <thead class="bg-gray-50/80">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 border-b border-gray-100 w-48 text-center text-xs font-bold text-gray-500">Hak Akses / Role</th>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                <tr v-for="(item, index) in users.data" :key="item.id" 
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="h-8 w-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-bold text-xs uppercase shadow-sm">
                                                {{ item.name.substring(0, 2) }}
                                            </div>
                                            <div class="min-w-0">
                                                <div class="font-semibold text-sm text-gray-800 truncate">{{ item.name }}</div>
                                                <div class="text-[11px] text-gray-400 leading-tight truncate mt-0.5">{{ item.email }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <div v-if="item.roles && item.roles.length > 0" class="flex items-center justify-center gap-1 flex-wrap">
                                            <span v-for="role in item.roles" :key="role.id" 
                                                ...
                                                :class="role.name === 'admin' ? 'bg-indigo-100 text-indigo-700 border border-indigo-200' : 'bg-gray-100 text-gray-600 border border-gray-200'">
                                                {{ role.name }}
                                            </span>
                                        </div>
                                        <span v-else class="text-[10px] text-gray-400 italic">User Biasa</span>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end gap-1">
                                            <button @click="openEdit(item)" class="p-1.5 text-gray-400 hover:text-amber-600 rounded-md hover:bg-amber-50 transition-all duration-150" title="Edit">
                                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                            </button>
                                            <button @click="deleteItem(item)" class="p-1.5 text-gray-400 hover:text-red-600 rounded-md hover:bg-red-50 transition-all duration-150" title="Hapus">
                                ...
                                <tr v-if="!users.data?.length">
                                    <td colspan="5" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-xs font-semibold text-gray-400">Belum ada pengguna</p>
                                            <p class="text-[10px] text-gray-300 mt-0.5">Silakan buat akun pengguna baru</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div class="px-6 py-3.5 border-t border-gray-100 bg-gray-50/30 flex items-center justify-between mt-auto">
                        ...
                        <nav v-if="users.links && users.links.length > 3" class="flex gap-1">
                ...
                <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-xl border border-gray-200 shadow-2xl w-full max-w-lg overflow-hidden">
                        <div class="px-6 py-4 flex justify-between items-center bg-emerald-600 shadow-sm relative z-10">
                            <h3 class="text-sm font-bold text-white tracking-wide">{{ editingItem ? 'Edit Pengguna' : 'Tambah Pengguna' }}</h3>
                        ...
                        <form @submit.prevent="submit" class="p-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="col-span-full">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Alamat Email *</label>
                                    <input v-model="form.email" type="email" 
                                        class="w-full rounded-lg border-gray-300 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition" 
                                        placeholder="email@interprima.co.id" required />
                                    <p v-if="form.errors.email" class="mt-2 text-[10px] font-bold text-rose-600 uppercase">{{ form.errors.email }}</p>
                                </div>
                                <div class="col-span-full">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Hak Akses / Role *</label>
                                    <select v-model="form.role" required
                                        class="w-full rounded-lg border-gray-300 text-sm focus:border-emerald-500 focus:ring-emerald-500 transition shadow-sm uppercase">
                                        <option value="" disabled>PILIH ROLE</option>
                                        <option v-for="role in roles" :key="role.id" :value="role.name">
                                            {{ role.name }}
                                        </option>
                                    </select>
                                    <p v-if="form.errors.role" class="mt-2 text-[10px] font-bold text-rose-600 uppercase">{{ form.errors.role }}</p>
                                </div>
                                
                                <div class="col-span-full mt-2 mb-1" v-if="editingItem">
                                    <p class="text-[10px] font-bold text-amber-600 uppercase bg-amber-50 p-2 rounded border border-amber-100">
                                        Kosongkan sandi di bawah jika tidak ingin mengubah tipe sandi / password saat ini!
                                    </p>
                                </div>

                                <div>
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Sandi / Password <span v-if="!editingItem">*</span></label>
                                    <input v-model="form.password" type="password" 
                                        class="w-full rounded-lg border-gray-300 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition" 
                                        placeholder="••••••••" :required="!editingItem" autocomplete="new-password" />
                                    <p v-if="form.errors.password" class="mt-2 text-[10px] font-bold text-rose-600 uppercase">{{ form.errors.password }}</p>
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Ulangi Sandi <span v-if="!editingItem">*</span></label>
                                    <input v-model="form.password_confirmation" type="password" 
                                        class="w-full rounded-lg border-gray-300 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition" 
                                        placeholder="••••••••" :required="!editingItem" autocomplete="new-password" />
                                    <p v-if="form.errors.password_confirmation" class="mt-2 text-[10px] font-bold text-rose-600 uppercase">{{ form.errors.password_confirmation }}</p>
                                </div>
                            </div>
