<script setup>
/**
 * Vendors Index Page - Standardized Table UI
 */
...
watch(perPage, (val) => {
    ...
    router.get(route('vendors.index'), { ...props.filters, per_page: val }, { 
        preserveScroll: true, 
        preserveState: true,
...
const submit = () => editingItem.value ? form.put(route('vendors.update', editingItem.value.id), { onSuccess: () => showModal.value = false }) : form.post(route('vendors.store'), { onSuccess: () => showModal.value = false });
...
const toggleAll = (e) => {
    if (e.target.checked) {
        selectedItems.value = props.vendors.data.map(i => i.id);
    } else {
        selectedItems.value = [];
    }
};
...
const bulkDelete = () => {
    confirmData.value = {
        title: 'Hapus Massal Vendor',
        message: `Hapus ${selectedItems.value.length} vendor yang dipilih?`,
        type: 'danger',
        onConfirm: () => {
            router.post(route('vendors.bulk-delete'), { ids: selectedItems.value }, {
                onSuccess: () => {
                    selectedItems.value = [];
...
const toggleSort = () => {
    ...
    router.get(route('vendors.index'), { ...props.filters, dir: dir }, { 
        preserveScroll: true, 
        preserveState: true,
        only: ['vendors', 'filters']
    });
...
const deleteItem = (item) => {
    confirmData.value = {
        title: 'Hapus vendor',
        message: `Apakah Anda yakin ingin menghapus vendor "${item.name}"? Semua data yang terkait mungkin akan terpengaruh.`,
        type: 'danger',
        onConfirm: () => {
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 leading-tight">Manajemen Vendor & Supplier</h2>
                    <p class="text-xs text-gray-500 mt-1">Kelola daftar kontak perusahaan penyedia aset atau perbaikan.</p>
                </div>
            </div>
        </template>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-h-[600px] transition-opacity duration-300" :class="{'opacity-40 pointer-events-none': isLoading}">
                    ...
                    <div class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-gray-50 to-white">
                        <div class="flex items-center gap-3 shrink-0">
                            ...
                            <div class="hidden sm:block">
                                <h3 class="text-sm font-bold text-gray-800">Daftar Vendor</h3>
                                <p class="text-[10px] text-gray-400 font-medium">{{ vendors.total || 0 }} vendor</p>
                            </div>
                        </div>
                    ...
                    <div v-if="vendors.data?.length > 0" class="px-6 py-3 border-b border-gray-100">
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-3 rounded-xl transition-all border"
                            ...
                            <div class="flex items-center gap-3">
                                <label class="flex items-center gap-2 cursor-pointer select-none">
                                    <input type="checkbox" @change="toggleAll" :checked="selectedItems.length === vendors.data?.length && vendors.data?.length > 0"
                        ...
                        <table class="min-w-full text-left">
                            <thead class="bg-gray-50/80">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 border-b border-gray-100 w-48 text-xs font-bold text-gray-500">Informasi Kontak</th>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                <tr v-for="(item, index) in vendors.data" :key="item.id" 
                                    ...
                                    <td class="px-2 py-4 text-center text-[11px] text-gray-400 font-bold tabular-nums">
                                        {{ (vendors.current_page - 1) * vendors.per_page + index + 1 }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="min-w-0">
                                            <div class="font-semibold text-sm text-gray-800 truncate">{{ item.name }}</div>
                                            <div class="text-[10px] text-gray-400 leading-tight truncate max-w-sm mt-0.5" v-if="item.contact_person">PIC: {{ item.contact_person }}</div>
                                            <div class="text-[10px] text-gray-400 leading-tight truncate max-w-sm mt-0.5" v-if="item.address"><span class="italic">{{ item.address }}</span></div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col gap-1">
                                            <div v-if="item.phone" class="inline-flex items-center gap-1.5 text-[11px] text-gray-600">
                                            ...
                                            <div v-if="item.email" class="inline-flex items-center gap-1.5 text-[11px] text-gray-600">
                                                <svg class="h-3 w-3 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                                {{ item.email }}
                                            </div>
                                            <div v-if="!item.phone && !item.email" class="text-[10px] text-gray-300 font-medium italic">Tidak ada kontak</div>
                                        </div>
                                    </td>
                                ...
                                <tr v-if="!vendors.data?.length">
                                    <td colspan="6" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-xs font-semibold text-gray-400">Belum ada vendor / supplier</p>
                                            <p class="text-[10px] text-gray-300 mt-0.5">Silakan tambahkan data rekanan baru</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div class="px-6 py-3.5 border-t border-gray-100 bg-gray-50/30 flex items-center justify-between mt-auto">
                        ...
                        <nav v-if="vendors.links && vendors.links.length > 3" class="flex gap-1">
            ...
            >
                <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-xl border border-gray-200 shadow-2xl w-full max-w-lg overflow-hidden">
                        <div class="px-5 py-3.5 flex justify-between items-center bg-emerald-600 shadow-sm relative z-10">
                            <h3 class="text-sm font-bold text-white tracking-wide">{{ editingItem ? 'Edit Vendor' : 'Tambah Vendor' }}</h3>
                        ...
                        <form @submit.prevent="submit" class="p-5">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div class="col-span-full">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Nama Vendor / PT *</label>
                                    <input v-model="form.name" type="text" 
                                        class="w-full rounded-lg border-gray-300 text-sm py-2 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition" 
                                        placeholder="Contoh: PT. Sumber Makmur" required />
                                    <p v-if="form.errors.name" class="mt-1 text-[10px] font-bold text-rose-600 uppercase">{{ form.errors.name }}</p>
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Contact Person</label>
                                    <input v-model="form.contact_person" type="text" 
                                        class="w-full rounded-lg border-gray-300 text-sm py-2 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition" 
                                        placeholder="Contoh: Bpk. Budi" />
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Telepon / WhatsApp</label>
                                    <input v-model="form.phone" type="text" 
                                        class="w-full rounded-lg border-gray-300 text-sm py-2 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition" 
                                        placeholder="Contoh: 08123456789" />
                                </div>
                                <div class="col-span-full">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Alamat Email</label>
                                    <input v-model="form.email" type="email" 
                                        class="w-full rounded-lg border-gray-300 text-sm py-2 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition" 
                                        placeholder="Contoh: cs@vendor.com" />
                                </div>
                                <div class="col-span-full">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Alamat Lengkap</label>
                                    <textarea v-model="form.address" rows="2" 
                                        class="w-full rounded-lg border-gray-300 text-sm py-2 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition resize-y"
                                        placeholder="Jalan, kelurahan, kota..."></textarea>
                                </div>
                                <div class="col-span-full">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Keterangan Singkat</label>
                                    <textarea v-model="form.description" rows="2" 
                                        class="w-full rounded-lg border-gray-300 text-sm py-2 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm transition resize-y"
                                        placeholder="Supplier laptop, alat tulis, dll."></textarea>
                                </div>
                            </div>
                            <div class="flex justify-end gap-2.5 pt-3 mt-4 border-t border-gray-100">
                                <button type="button" @click="showModal = false" 
                                    class="px-4 py-2 text-[11px] font-bold text-gray-500 uppercase tracking-wider rounded-lg border border-gray-200 hover:bg-gray-50 transition">
                                    Batal
                                </button>
                                <button type="submit" 
                                    class="bg-emerald-600 text-white px-5 py-2 rounded-lg text-[11px] font-bold hover:bg-emerald-700 shadow-sm uppercase tracking-wider transition-all disabled:opacity-50" 
